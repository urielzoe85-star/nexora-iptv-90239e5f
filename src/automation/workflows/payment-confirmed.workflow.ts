...
import { createIptvSubscription, composeIptvDelivery, dispatchIptvDelivery, renewIptvSubscription } from "../actions/iptv.actions";
...
export const paymentConfirmedWorkflow: WorkflowDefinition = {
  ...
  steps: [
    ...
    },
    {
      ...
      when: (ctx) => Boolean((ctx.outputs["validate:order"] as any)?.isRenewal),
      run: async (ctx) => {
        const v = ctx.outputs["validate:order"] as {
          orderId: string; email: string; renewalAccountId: string; renewalMonths: number;
        };
