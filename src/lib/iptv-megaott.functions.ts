/* eslint-disable @typescript-eslint/no-explicit-any -- provider JSON is normalized at boundaries. */
...
import {
  megaottConnector,
  megaottRawCall,
  megaottRealProvisioningEnabled,
  pingMegaott,
  resolveMegaottConfig,
} from "@/integration-hub/connectors/iptv/megaott.adapter";
...
async function log(
  ...
  try {
    await sb.from("iptv_logs").insert({ actor_id: actorId, action, message, payload, account_id });
  } catch {
    /* noop */
  }
}
...
function redactSensitive(value: unknown): unknown {
  ...
  for (const [key, item] of Object.entries(value as Record<string, unknown>)) {
    out[key] = /(password|pass|token|authorization|secret)/i.test(key)
      ? "***"
...
async function debugTrace(
  ...
  try {
    await sb.from("integration_debug_logs").insert({
      ...
      method: t.method,
      url: t.url,
      request_headers: redactSensitive(t.requestHeaders ?? {}),
      request_body: redactSensitive(t.requestBody ?? null),
      status: t.status,
      response_body: redactSensitive(t.responseBody ?? null),
      duration_ms: t.durationMs,
      attempts: t.attempts,
      ok: t.ok,
      error: safeErrorMessage(t.error),
    });
  } catch {
    /* never fail an op for a debug log write */
  }
}
...
export const megaottPing = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await log(
      ...
      context.userId,
      "megaott.ping",
      r.ok ? "ok" : safeErrorMessage(r.error.message),
      r.ok ? { status: r.value.status, durationMs: r.value.durationMs } : { kind: r.error.kind },
    );
    if (!r.ok)
      return {
        ok: false as const,
        error: safeErrorMessage(r.error.message),
        kind: r.error.kind,
    ...
    return {
      ...
      authenticated: r.value.authenticated,
      responseValid: r.value.responseValid,
      account: r.value.account,
    };
  });

// Explicit health-check name for monitoring and runbooks. It is read-only:
// the only upstream request is GET /api/v1/user, never subscription create.
export const megaottHealth = createServerFn({ method: "POST" })
  .middleware([requireNccUnlock])
  .inputValidator((d: unknown) => z.object({ url: z.string().url().optional() }).parse(d ?? {}))
  .handler(async ({ data, context }) => {
    const sb = await admin(context.userId);
    ...
    await log(
      ...
      r.ok
        ...
        : { kind: r.error.kind, status: r.error.status ?? null },
    );
    if (!r.ok)
      return {
        ok: false as const,
...
/** Admin-only retry for a paid order whose provisioning failed or needs review. */
export const retryMegaottProvisioning = createServerFn({ method: "POST" })
  .middleware([requireNccUnlock])
  .inputValidator((d: unknown) =>
    z
      .object({
        order_id: z.string().uuid(),
        confirm_ambiguous: z.boolean().optional().default(false),
      })
      .parse(d),
  )
  .handler(async ({ data, context }) => {
    ...
    if (!["paid", "completed"].includes(String(order.status))) {
      throw new Error("Retry refusé : paiement non confirmé");
    }
    const meta = (order.metadata ?? {}) as Record<string, any>;
    const delivery = meta.iptv_delivery ?? null;
    const provisioning = meta.iptv_provisioning ?? {};
    if (provisioning.provider_creation_possible && !data.confirm_ambiguous) {
      throw new Error(
        "Retry refusé : création MegaOTT potentiellement déjà effectuée. Synchronisez d'abord ou confirmez explicitement.",
      );
    }
    if (delivery?.iptv_account_id && delivery?.megaott_subscription_id) {
      return { ok: true as const, skipped: true as const, reason: "already_provisioned" };
    }
    await sb
      .from("orders")
      .update({
    ...
    const run = await automationApi.run(
      "payment-confirmed",
      {
        ...
        provider: "admin_provisioning_retry",
        forced: true,
      },
      context.userId,
    );
...
/** Admin-only read/sync of an existing remote subscription; never creates one. */
export const syncMegaottOrder = createServerFn({ method: "POST" })
  .middleware([requireNccUnlock])
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: order, error } = await sb
      ...
      .maybeSingle();
    if (error) throw new Error(error.message);
    const accountId = (order?.metadata as any)?.iptv_delivery?.iptv_account_id;
    if (!accountId) return { ok: false as const, error: "Aucun compte IPTV associé" };
    const { data: account } = await sb
      .from("iptv_accounts")
      .select("id, metadata, megaott_subscription_id, expires_at")
      .eq("id", accountId)
      .maybeSingle();
    const remoteId = account?.megaott_subscription_id ?? (account?.metadata as any)?.remote_user_id;
    if (!remoteId) return { ok: false as const, error: "Compte sans identifiant MegaOTT" };
    const r = await getConnector().getUser(String(remoteId));
    if (!r.ok)
      return { ok: false as const, error: safeErrorMessage(r.error.message), kind: r.error.kind };
    const meta = {
      ...((account?.metadata ?? {}) as Record<string, unknown>),
      remote_status: r.value.status,
      last_sync_at: new Date().toISOString(),
      dns_link: r.value.dnsLink ?? (account?.metadata as any)?.dns_link,
      portal_link: r.value.portalLink ?? (account?.metadata as any)?.portal_link,
    };
    await sb
      .from("iptv_accounts")
      .update({
        metadata: meta,
        expires_at: r.value.expiresAt ?? account?.expires_at ?? null,
        status: r.value.status,
      })
      .eq("id", accountId);
    return { ok: true as const, status: r.value.status, expiresAt: r.value.expiresAt ?? null };
  });
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!r.ok) {
      await log(
        sb,
        context.userId,
        "megaott.create.failed",
        safeErrorMessage(r.error.message),
        { kind: r.error.kind },
        data.account_id,
      );
      return { ok: false as const, error: safeErrorMessage(r.error.message), kind: r.error.kind };
    ...
    const meta = {
      ...
      package_id: r.value.packageId ?? data.package_id ?? acc.bouquet ?? null,
      provider: "megaott",
      last_sync_at: new Date().toISOString(),
    };
    await sb
      ...
      .update({
        metadata: meta,
        password: r.value.password ?? acc.password ?? null,
        megaott_subscription_id: r.value.providerUserId,
        dns_link: r.value.dnsLink ?? null,
        portal_link: r.value.portalLink ?? null,
        expires_at: r.value.expiresAt ?? acc.expires_at ?? null,
        status: "active",
      })
      .eq("id", data.account_id);
    await log(
...
export const megaottSuspendRemote = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await log(
      sb,
      context.userId,
      r.ok ? "megaott.suspend.ok" : "megaott.suspend.failed",
      r.ok ? remoteId : safeErrorMessage(r.error.message),
      {},
      data.account_id,
    );
    if (!r.ok) return { ok: false as const, error: safeErrorMessage(r.error.message) };
...
export const megaottReactivateRemote = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await log(
      sb,
      context.userId,
      r.ok ? "megaott.reactivate.ok" : "megaott.reactivate.failed",
      r.ok ? remoteId : safeErrorMessage(r.error.message),
      {},
      data.account_id,
    );
...
export const megaottExtendRemote = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await log(
      sb,
      context.userId,
      r.ok ? "megaott.extend.ok" : "megaott.extend.failed",
      r.ok ? remoteId : safeErrorMessage(r.error.message),
      { next },
      data.account_id,
    );
    ...
    await sb
      .from("iptv_accounts")
      .update({ status: "active", expires_at: next })
      .eq("id", data.account_id);
    return { ok: true as const, user: r.value };
  });
...
export const megaottSyncRemote = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!r.ok) {
      ...
      return { ok: false as const, error: safeErrorMessage(r.error.message) };
    }
    const meta = {
      ...(acc?.metadata ?? {}),
      remote_status: r.value.status,
...
const SubscriptionSchema = z
  .object({
    ...
    package: z.union([z.string(), z.number()]),
  ...
  .refine((value) => (value.type === "m3u" ? Boolean(value.username) : Boolean(value.mac)), {
    message: "Un username est requis pour M3U, une adresse MAC pour MAG/Enigma",
    path: ["username"],
  });
...
export const megaottCreateSubscription = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    if (!megaottRealProvisioningEnabled()) {
      return {
        ok: false as const,
        error: "MEGAOTT real provisioning is disabled",
        status: null,
        response: null,
      };
    }
    ...
    const body: Record<string, unknown> = {
      ...
      ...(data.template ? { template_id: data.template } : {}),
      ...(data.max_connections !== undefined ? { max_connections: data.max_connections } : {}),
      ...(data.forced_country ? { forced_country: data.forced_country } : {}),
      ...(data.adult !== undefined ? { adult: data.adult } : {}),
      ...(data.whatsapp_telegram ? { whatsapp_telegram: data.whatsapp_telegram } : {}),
      ...(data.enable_vpn !== undefined ? { enable_vpn: data.enable_vpn } : {}),
    ...
    const cfg = await resolveMegaottConfig();
    const path =
      (cfg.ok && (cfg.value.endpoints as any)?.createSubscription) || "/api/v1/subscriptions";

    const trace = await megaottRawCall({ path, method: "POST", body: form.toString() });
    ...
    if (!trace.ok) {
      await log(
        sb,
        context.userId,
        "megaott.subscription.failed",
        safeErrorMessage(trace.error ?? "error"),
        {
          status: trace.status,
      ...
      return {
        ok: false as const,
        error: safeErrorMessage(trace.error ?? "Erreur inconnue"),
        status: trace.status,
    ...
    const created = {
      ...
      package:
        r.package?.name ?? r.package_name ?? r.package?.id ?? r.package_id ?? data.package ?? null,
      template:
        r.template?.name ??
        r.template_name ??
        r.template?.id ??
    ...
    const { data: row, error: insErr } = await sb
      ...
      .insert({
        ...
        package: created.package ? String(created.package) : null,
        dns_link: created.dns_link,
        dns_link_samsung_lg: created.dns_link_for_samsung_lg,
        portal_link: created.portal_link,
        mac_address: created.mac,
        max_connections: data.max_connections ?? 1,
        megaott_subscription_id: created.id ? String(created.id) : null,
        ...
        metadata: {
          ...
          portal_link: created.portal_link,
          raw_response: redactSensitive(trace.responseBody),
    ...
    if (insErr) {
      await log(sb, context.userId, "megaott.subscription.persist_failed", insErr.message, {
        created,
      });
    } else {
      await log(
...
export const getMegaottPanelUrl = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const cfg = await resolveMegaottConfig();
    if (!cfg.ok)
      return { ok: false as const, error: safeErrorMessage(cfg.error.message), url: null };
    try {
...
const DeliverySchema = z.object({
  ...
  max_connections: z.coerce.number().int().min(1).max(20).optional().nullable(),
...
export const saveMegaottDelivery = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: account, error: iErr } = await sb
      .from("iptv_accounts")
      .insert({
        provider_id: provider?.id ?? null,
    ...
    const delivery = buildDeliveryFromAccount({
      ...
      previous: (meta as any).iptv_delivery ?? null,
    });
