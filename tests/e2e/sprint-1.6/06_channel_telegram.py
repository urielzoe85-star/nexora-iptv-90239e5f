#!/usr/bin/env python3
"""Sans telegram_chat_id, telegram = skipped."""
...
def main() -> int:
    ...
    try:
        ...
        ch = (d.get("channels_sent") or {}).get("telegram")
        assert ch is not None, "channels_sent.telegram absent"
        assert ch.get("ok") in (False, None), f"telegram inattendu ok=True : {ch}"
        print(f"OK — telegram skipped ({ch.get('error')})")
        return 0
    finally:
